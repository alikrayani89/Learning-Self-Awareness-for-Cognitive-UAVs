...
% plotting the colors
imagesc(confpercent);
% % h.position=[508   559   758   193];
% % title(sprintf('Accuracy: %.2f%%', 100*trace(confmat)/sum(confmat(:))));
% % ylabel('Graph a'); xlabel('Graph b');
xlabel('$\bf{Graph \ b}$', 'FontSize',10, 'interpreter','latex');
ylabel('$\bf{Graph \ a}$', 'FontSize',10, 'interpreter','latex');
% set the colormap
...
set(gcf,'Position', [508   559   758   193]) % set the position of the current figure
